...
One method to do so is to use the CLI:
    * Acquire the Supra CLI, see https://supra.dev/cli-tools/supra-cli/use-cli/install-supra-cli
...
async def main(moon_coin_path: str):
    ...
    if SupraCLIWrapper.does_cli_exist():
        SupraCLIWrapper.compile_package(moon_coin_path, {"MoonCoin": alice.address()})
    else:
